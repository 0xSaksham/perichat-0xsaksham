...
} from "@/app/utils/chatService";
import Rightbar from "@/app/components/RightBar";
import  ContactsList  from "@/app/components/chat/ContactsList";
...
const ChatsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = chatService.subscribeToMessages(
      ...
      (updatedMessage) => {
        if (
          selectedContact &&
          (updatedMessage.sender_id === selectedContact.id ||
            updatedMessage.receiver_id === selectedContact.id)
        ) {
          setMessages((prev) =>
            prev.map((msg) =>
  ...
  return (
    <div className="flex h-full relative">
      <div
        className={`${!showContactsOnMobile ? "hidden md:block" : "block"} w-full md:w-80 lg:w-96 h-full overflow-hidden`}
      >
        <ContactsList
          contacts={contacts}
      ...
      <div
        className={`${showContactsOnMobile ? "hidden md:flex" : "flex"} flex-col flex-1 h-full overflow-hidden`}
      >
        {selectedContact && (
      ...
      {selectedContact && (
        <div
          className={`${showContactsOnMobile ? "hidden md:block" : "block"} h-full overflow-hidden`}
        >
