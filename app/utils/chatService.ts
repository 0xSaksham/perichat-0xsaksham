...
import { UserSentState } from "@/app/components/Contact";
    ...
    try {
      ...
      const contacts = await Promise.all(
        profilesData.map(async (profile: any) => {
          ...
          const latestMessage =
            messageData && messageData[0] ? messageData[0].content : "";
          const lastMessageDate =
            messageData && messageData[0]
              ? formatMessageDate(messageData[0].created_at)
              : "";
          // Store the actual timestamp for sorting
          const lastMessageTimestamp =
            messageData && messageData[0] ? messageData[0].created_at : "";

          let userSentState: UserSentState | undefined;
          if (messageData && messageData[0]) {
            if (messageData[0].sender_id === userId) {
              userSentState =
                messageData[0].status === "read"
                  ? UserSentState.READ
